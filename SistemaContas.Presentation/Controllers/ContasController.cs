...
namespace SistemaContas.Presentation.Controllers
{
    ...
    public class ContasController : Controller
    {
        ...
        public IActionResult Edicao(Guid id)
        {
            ...
            try
            {
                var usarioModel = JsonConvert.DeserializeObject<UsuarioModel>(User.Identity.Name);

                var contaRepository = new ContaRepository();
                var conta = contaRepository.ObterPorId(id);

                if (conta != null && conta.IdUsuario == usarioModel.IdUsuario)
                {
            ...
            {
                TempData["MensagemErro"] = $"Falha ao editar contas: {e.Message}";
        ...
        public IActionResult Edicao(ContasEdicaoModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    TempData["MensagemErro"] = $"Falha ao atualizar contas: {e.Message}";
        ...
        private List<SelectListItem> ObterItensCategoria()
        {
            ...
            try
            {
                ...
                var categorias = categoriaRepository.ObterTodos(usarioModel.IdUsuario);
